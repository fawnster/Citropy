...
import type { ComputerAction, ComputerActivity, ComputerCapabilities, ComputerDisplay, ComputerFrame, ComputerState } from "../shared/computer.ts";
...
export async function startComputer(threadId: string, user = false): Promise<ComputerState> {
  ...
  try {
    ...
    if (version !== revision) throw new Error("Computer use was stopped.");
    state = { ...state, ...result, status: "active", startedAt: Date.now(), error: undefined };
    touch();
...
export function computerScreenshot(threadId: string, displayId?: string, maxWidth = 1600, preview = false): Promise<ComputerFrame> {
  owner(threadId, preview);
  const version = revision;
  const operation = queue.catch(() => {}).then(async () => {
    if (version !== revision) throw new Error("Computer session changed.");
    owner(threadId, preview);
    const display = state.displays.find((entry) => entry.id === (displayId || state.displays[0]?.id));
    if (!display) throw new Error("Select a shared screen.");
    if (!Number.isInteger(maxWidth) || maxWidth < 320 || maxWidth > 2560) throw new Error("Use an image width from 320 to 2560 pixels.");
    const image = await desktopRequest<{ image: string; width: number; height: number }>("computer.screenshot", { displayId: display.id, maxWidth });
    if (version !== revision) throw new Error("Computer session changed.");
    const frame: ComputerFrame = { ...image, id: randomUUID(), displayId: display.id, sourceWidth: display.width, sourceHeight: display.height, capturedAt: Date.now() };
...
function inputFor(input: ComputerAction): Record<string, unknown> {
  ...
  if ("frameId" in input) {
    ...
    out.displayId = frame.displayId;
    out.x = finite(input.x, 0, frame.width - 1, "x coordinate") * frame.sourceWidth / frame.width;
    out.y = finite(input.y, 0, frame.height - 1, "y coordinate") * frame.sourceHeight / frame.height;
    if (input.action === "drag") {
      out.toX = finite(input.toX, 0, frame.width - 1, "target x") * frame.sourceWidth / frame.width;
      out.toY = finite(input.toY, 0, frame.height - 1, "target y") * frame.sourceHeight / frame.height;
    }
