...
Lime builds use a prerelease version like `0.2.1-lime.42`, one patch above the current stable version, so they sort above the release they came from. The app detects the channel from its version string, and treats the older `-lemon` suffix as the same channel. The installer accepts `--channel=lemon` as an alias for Lime, and the old `lemon` release keeps a version file that advertises the first Lime build, so a build installed before the rename updates into Lime on its own. Lime is the same app as stable, with the same bundle identifier, data directory (`~/.citropy`), Electron profile, and port, so switching channels replaces the installed build and keeps conversations, settings, and terminals. The Lime build is marked by a lime channel badge beside the title and its own lime application icon, while the Citropy mark stays the same yellow lemon. The release assets keep their own names (`Citropy-lime-*`) so the installer can tell the channels apart. Lime updates always go through the installer on every platform, and the installer reopens the app when it finishes. Settings offers the channel switch: it downloads the installer for the other channel, clears the variables of the running build, and runs it, which replaces the app and reopens it.
...
Pushing to `main` also runs the **Lime** workflow. It runs the same checks first, then builds Linux, macOS, and Windows from that commit with a version like `0.2.1-lime.42`, writes `version.json` with that version, and refreshes the rolling `lime` prerelease. Installers download the zips and `version.json` first and `SHA256SUMS` last, so a half-finished publish shows up as a checksum failure rather than a mismatched install. `scripts/install.sh --channel=lime` installs it, and `--channel=lemon` keeps working as an alias for builds that predate the rename. Lime releases are automatic and never touch the stable `v<version>` tags.
