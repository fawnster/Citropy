...
Lemon builds use a prerelease version like `0.2.1-lemon.42`, one patch above the current stable version, so they sort above the release they came from. The app detects the channel from its version string. Lemon is the same app as stable, with the same bundle identifier, data directory (`~/.citropy`), Electron profile, and port, so switching channels replaces the installed build and keeps conversations, settings, and terminals. The release assets keep their own names (`Citropy-lemon-*`) so the installer can tell the channels apart. Lemon updates always go through the installer on every platform, and the installer reopens the app when it finishes. Settings offers the channel switch: it downloads the installer for the other channel, clears the variables of the running build, and runs it, which replaces the app and reopens it.
