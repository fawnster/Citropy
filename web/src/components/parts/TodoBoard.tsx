...
export function TodoBoard({ partId, items }: { partId: string; items: TodoItem[] }) {
  ...
  return (
    <section className="todo" aria-label={t("Plan")}>
      ...
      {!open && current && <div className="todo-current"><CircleDot size={14} aria-hidden="true" /><span>{current.text}</span></div>}
      <ul id={`plan-${partId}`} className="todo-list" hidden={!open}>
