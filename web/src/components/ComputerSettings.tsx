...
export function ComputerSettings() {
  ...
  return <div className="feature-stack">
    ...
    <div className="computer-support"><Monitor size={22} className="panel-icon-computer" /><div><strong>{capabilities?.available ? capabilities.backend === "wayland-portal" ? t("Wayland desktop portal") : t("X11 desktop control") : t("Desktop support")}</strong><p>{capabilities?.available ? t("Screen capture, mouse input, keyboard shortcuts, text entry, and dragging are available.") : capabilities?.reason ?? t("Checking this computer…")}</p></div>{capabilities?.available ? <Check size={18} className="text-ok" /> : <button className="icon-btn" aria-label={t("Check computer support")} onClick={() => setRefresh((value) => value + 1)}><RefreshCw size={17} /></button>}</div>
    <h2 className="settings-group-heading">{t("How sessions work")}</h2>
    <div className="settings-group"><div className="setting-row"><span><strong>{t("You choose what to share")}</strong><small>{t("Open Computer from the workspace panel, or ask a provider to use your desktop. Wayland asks you to select screens in its own sharing dialog.")}</small></span></div><div className="setting-row"><span><strong>{t("One conversation in control")}</strong><small>{t("Other conversations cannot send input to the active session. Plan only mode can view the screen. Other actions follow the conversation's permission setting.")}</small></span></div><div className="setting-row"><span><strong>{t("Pause or stop at any time")}</strong><small>{t("The Computer panel and title bar show when sharing is active. Ctrl+Alt+Escape stops control where supported. Sessions close after five minutes without actions, and when their conversation is stopped or finished.")}</small></span></div><div className="setting-row"><span><strong>{t("Screenshots stay in the session")}</strong><small>{t("Citropy keeps preview images in memory. Providers receive requested screenshots as tool results. Recent activity records action names, without storing typed text.")}</small></span></div></div>
