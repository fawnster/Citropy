...
import { ChevronDown, LockKeyhole, Star } from "lucide-react";
...
export function ModelPicker({ value, fallback, label, onChange, disabled = false, allowConversation = false, automaticLabel, lockedProvider, className = "model-picker-trigger", buttonRef }: {
  ...
  const catalog = lockedProvider ? providers.find((entry) => entry.id === lockedProvider) : available.find((entry) => entry.id === browsing) ?? available[0];
  ...
  const catalogs = favoritesView ? available.filter((entry) => !lockedProvider || entry.id === lockedProvider) : catalog ? [catalog] : [];
  return <Menu
    width={340}
    ...
    controls={<>
      <div className="model-picker-toolbar">
        {lockedProvider && catalog ? <button className="model-picker-locked" type="button" aria-label={`${catalog.label} · ${t("Provider locked")}`} title={`${catalog.label} · ${t("Provider locked")}`} aria-pressed={!favoritesView} onClick={() => setBrowsing(catalog.id)}>
    ...
    items={[
      ...(automatic && !favoritesView ? [{ id: "conversation", label: automatic, selected: !value, onSelect: () => onChange(null) }] : []),
      ...catalogs.flatMap((source) => source.models.filter((entry) => !favoritesView || favorites.some((favorite) => favorite.provider === source.id && favorite.model === entry.id)).map((entry) => ({
        ...
        onSelect: () => onChange({ provider: source.id, model: entry.id }),
        action: {
    ...
    trigger={({ toggle, id, open }) => <button
      ...
      aria-expanded={open}
      disabled={disabled}
      onClick={() => { if (!open) { setBrowsing(choice?.provider); send({ t: "providers.refresh" }); } toggle(); }}
    >
