...
export class SshEnvironments {
  ...
  async runtimeArchive(platform, signal) {
    ...
    const response = await fetch(`https://nodejs.org/download/release/v${nodeVersion}/node-v${nodeVersion}-${platform}.tar.gz`, { signal: AbortSignal.any([signal, AbortSignal.timeout(180000)]), redirect: "error" });
